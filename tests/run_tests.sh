...
echo "Parse Failures:"
if [ -d "$SCRIPT_DIR/output/parse-failures/" ]; then
    echo "Found parse failures:"
    find "$SCRIPT_DIR/output/parse-failures/" -type f -name "*.json" -exec cat {} \;
else
    echo "No parse failures found"
fi

echo -e "\nBad Logs:"
if [ -d "$SCRIPT_DIR/output/bad-logs/" ]; then
    echo "Found bad logs:"
    find "$SCRIPT_DIR/output/bad-logs/" -type f -name "*.json" -exec cat {} \;
else
    echo "No bad logs found"
fi

echo -e "\nInvalid HTTP Methods:"
if [ -d "$SCRIPT_DIR/output/invalid-methods/" ]; then
    echo "Found invalid HTTP methods:"
    find "$SCRIPT_DIR/output/invalid-methods/" -type f -name "*.json" -exec cat {} \;
else
    echo "No invalid HTTP methods found"
fi

echo -e "\nTemplate Errors:"
if [ -d "$SCRIPT_DIR/output/template-errors/" ]; then
    echo "Found template errors:"
    find "$SCRIPT_DIR/output/template-errors/" -type f -name "*.json" -exec cat {} \;
else
    echo "No template errors found"
fi

echo -e "\nEmpty User Agents:"
if [ -d "$SCRIPT_DIR/output/empty-user-agents/" ]; then
    echo "Found empty user agents:"
    find "$SCRIPT_DIR/output/empty-user-agents/" -type f -name "*.json" -exec cat {} \;
else
    echo "No empty user agents found"
fi

echo -e "\nDuplicate Source Addresses:"
if [ -d "$SCRIPT_DIR/output/duplicate-sources/" ]; then
    echo "Found duplicate source addresses:"
    find "$SCRIPT_DIR/output/duplicate-sources/" -type f -name "*.json" -exec cat {} \;
else
    echo "No duplicate source addresses found"
fi

echo -e "\nProcessed Logs:"
if [ -d "$SCRIPT_DIR/output/processed-logs/" ]; then
    echo "Found processed logs:"
    find "$SCRIPT_DIR/output/processed-logs/" -type f -name "*.json" -exec cat {} \;
else
